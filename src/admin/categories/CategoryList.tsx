  ...
  const addCategory = async () => {
    ...
    try {
      const res = await fetch('/api/categories', {
        ...
        body: JSON.stringify({ name: newName.trim(), name_en: newNameEn.trim() })
      });
      if (res.ok) {
        setNewName('');
        setNewNameEn('');
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{t("admin.category_management")}</h1>
          <p className="text-slate-500 mt-1">{t("admin.category_desc")}</p>
        </div>
      </div>
